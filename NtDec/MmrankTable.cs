...
namespace NtDec
{
    internal static class MmrankTable
    {
        ...
        {
            //WTF dude @Icem
            ushort k = MemoryMarshal.Cast<byte, ushort>(data)[0];
            if (SpecialMapping.TryGetValue(k, out var v))
            {
                data[0] = (byte)(v >> 8);
                data[1] = (byte)(v & 0xff);
                return true;
        ...
        public static bool OnsLocaleIsTwoBytes(in Span<byte> data)
        {
            return data[0] > 0x81 && data[0] != 0xff; //20130317
            //return (data[0] & 0xe0) == 0xe0 || (data[0] & 0xe0) == 0x80; //SHIFT-JIS
